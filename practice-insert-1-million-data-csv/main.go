...
const dbConnString = "root:secret@tcp(localhost:3306)/test"
...
func doTheJob(workerIndex, counter int, db *sql.DB, values []interface{}) {
	for {
		...
		func(outerError *error) {
			...
			query := fmt.Sprintf("INSERT INTO domain (%s) VALUES (%s)", strings.Join(dataHeaders, ","), strings.Join(generateQuestionMark(len(dataHeaders)), ","))
